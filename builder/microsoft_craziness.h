...
struct Find_Result {
    ...
    wchar_t *windows_sdk_version_specific  = NULL;
...
void free_resources(Find_Result *result) {
    ...
    free(result->windows_sdk_version_specific);
...
void win10_best(wchar_t *short_name, wchar_t *full_name, Version_Data *data) {
    ...
    data->best_name = _wcsdup(short_name);
...
void find_windows_kit_root(Find_Result *result) {
    ...
    if (windows10_root) {
        ...
        if (data.best_name) {
            ...
            result->windows_sdk_root = _wcsdup(windows10_root);
            result->windows_sdk_version_specific = data.best_name;
            return;
        }
    }
    ...
    if (windows8_root) {
        ...
        if (data.best_name) {
            ...
            result->windows_sdk_root = _wcsdup(windows8_root);
            result->windows_sdk_version_specific = data.best_name;
            return;
        }
    }
...
Find_Result find_visual_studio_and_windows_sdk() {
    ...
    if (result.windows_sdk_root) {
        result.windows_sdk_um_library_path     = concat(result.windows_sdk_root, L"Lib\\", result.windows_sdk_version_specific, L"\\um\\x64");
        result.windows_sdk_ucrt_library_path   = concat(result.windows_sdk_root, L"lib\\", result.windows_sdk_version_specific, L"\\ucrt\\x64");
